...
final class aeg_NM_NoticesManager {
	...
	const GLOBAL_DISMISS_OPTION = 'aeg-dismissed-notices';
	...
	private function dismiss_global_notice( $notice_id ) {
		$dismissed_notices = get_option( self::GLOBAL_DISMISS_OPTION );
		...
		update_option( self::GLOBAL_DISMISS_OPTION, $new_option, false );
	}

	private function is_dismissed( aeg_NM_Notice $notice ) {
		$is_dismissed = false;

		switch( $notice->get_dismiss_mode() ) {
			case aeg_NM_Notice::DISMISS_GLOBAL:
				$dismissed_notices = get_option( self::GLOBAL_DISMISS_OPTION );

				if( ! $dismissed_notices || in_array( $notice->get_id(), $dismissed_notices)) {
					$is_dismissed = true;
				}
				break;

			// todo add check for user dismiss
